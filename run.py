...
def main():
    if not sys.version_info >= (3, 4):
        print("Python 3.4+ is required. This version is %s" % sys.version.split()[0])
        print("Attempting to locate python 3.4...")
        ...
        if sys.platform.startswith('win'):
            try:
                subprocess.check_output('py -3.4 -c "exit()"', shell=True)
                pycom = 'py -3.4'
            except:

                try:
                    subprocess.check_output('python3 -c "exit()"', shell=True)
                    pycom = '/opt/python3/bin/python3'
                except:
                    pass
            ...
            try:
                pycom = subprocess.check_output(['which', 'python3.4']).strip().decode()
            except:
                pass
        ...
        print("Please run the bot using python 3.4")
    ...
    while tryagain:
        try:
            from autologin import AutoLoginBot
            AutoLoginBot().run()
